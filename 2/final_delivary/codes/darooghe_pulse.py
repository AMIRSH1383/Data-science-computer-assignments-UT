...
def generate_transaction_event(is_historical=False, timestamp_override=None):
    event_time = (
        timestamp_override if timestamp_override else datetime.datetime.now(datetime.UTC)
    )
...
def continuous_event_production(producer, topic, base_rate):
    while True:
        ...
        current_hour =  datetime.datetime.now(datetime.UTC)
        multiplier = peak_factor if 9 <= current_hour.hour < 18 else 1.0
